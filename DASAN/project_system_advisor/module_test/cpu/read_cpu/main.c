...
int main(void)
{
	...
	int ret = 0;
	int pid;
	double user_use_cpu_percent = 0;
	double kernel_use_cpu_percent = 0;

	puts("enter pid for test");
	scanf("%d", &pid);
	...
	while(1)
	{
		...
		printf("usage of process(%d) is %f %%\n", pid, 8*(user_use_cpu_percent +  kernel_use_cpu_percent));
